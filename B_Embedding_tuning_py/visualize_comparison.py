import os

import matplotlib.pyplot as plt
import pandas as pd

from config import EVAL_DIR

# Windows 기본 한글 폰트 (없으면 한글이 네모(□)로 깨짐)
plt.rcParams["font.family"] = "Malgun Gothic"
plt.rcParams["axes.unicode_minus"] = False

DETAIL_PATH = os.path.join(EVAL_DIR, "results_B_embedding_comparison_all.csv")
SUMMARY_PATH = os.path.join(EVAL_DIR, "results_B_embedding_comparison_summary.csv")
OUTPUT_PATH = os.path.join(EVAL_DIR, "embedding_comparison.png")


def _short_name(model_name):
    # "intfloat/multilingual-e5-large" -> "multilingual-e5-large" (그래프 라벨용)
    return model_name.split("/")[-1]


def main():
    df_all = pd.read_csv(DETAIL_PATH)
    summary = pd.read_csv(SUMMARY_PATH).sort_values("mean_bertscore_f1", ascending=False)

    labels = [_short_name(m) for m in summary["embedding_model"]]

    fig, axes = plt.subplots(1, 3, figsize=(18, 5.5))

    # 1) 모델별 평균 BERTScore F1
    ax = axes[0]
    bars = ax.bar(labels, summary["mean_bertscore_f1"], color="#4C72B0")
    ax.set_title("모델별 평균 BERTScore F1")
    ax.set_ylabel("BERTScore F1")
    ax.set_ylim(0, max(summary["mean_bertscore_f1"]) * 1.2)
    plt.setp(ax.get_xticklabels(), rotation=30, ha="right", fontsize=9)
    for bar, value in zip(bars, summary["mean_bertscore_f1"]):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height(), f"{value:.3f}",
                 ha="center", va="bottom", fontsize=9)

    # 2) 모델별 평균 응답시간
    ax = axes[1]
    bars = ax.bar(labels, summary["mean_response_time_sec"], color="#DD8452")
    ax.set_title("모델별 평균 응답시간 (초)")
    ax.set_ylabel("초")
    plt.setp(ax.get_xticklabels(), rotation=30, ha="right", fontsize=9)
    for bar, value in zip(bars, summary["mean_response_time_sec"]):
        ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height(), f"{value:.1f}s",
                 ha="center", va="bottom", fontsize=9)

    # 3) 모델별 질문 단위 F1 분포 (boxplot)
    ax = axes[2]
    order = summary["embedding_model"].tolist()
    data = [df_all.loc[df_all["embedding_model"] == m, "bertscore_f1"].values for m in order]
    ax.boxplot(data, tick_labels=[_short_name(m) for m in order])
    ax.set_title("모델별 질문 단위 F1 분포")
    ax.set_ylabel("BERTScore F1")
    plt.setp(ax.get_xticklabels(), rotation=30, ha="right", fontsize=9)

    fig.suptitle("임베딩 모델 비교 (자동차관리법 RAG)", fontsize=14)
    fig.tight_layout(rect=[0, 0, 1, 0.95])
    fig.savefig(OUTPUT_PATH, dpi=150)
    print(f"그래프 저장 완료: {OUTPUT_PATH}")

    plt.show()


if __name__ == "__main__":
    main()
