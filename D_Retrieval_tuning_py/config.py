...
# CONFIG — Retrieval 담당 (D) — tuning
# 실험 변수: 검색 전략. baseline dense(F1 0.6039) 포함 5가지 비교 결과
# hybrid(BM25 0.3/Dense 0.7)이 최고(F1 0.6385) — 최종 채택 (compare_retrieval.py 결과 참고)
# LLM/임베딩/chunk_size는 baseline과 동일하게 유지
CONFIG = {
    "run_name": "D_retrieval_tuning",
...
# 캐시 (재실행 시 중복 계산 방지)
CACHE_DIR = PROJECT_ROOT / "D_Retrieval_tuning_py" / ".cache"
