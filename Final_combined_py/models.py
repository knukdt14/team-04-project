...
def build_llm(llm_model_name=None, lora_adapter_path=None):
    ...
    lora_adapter_path = lora_adapter_path if lora_adapter_path is not None else CONFIG.get("lora_adapter_path")
    ...
    if lora_adapter_path:
        ...
        print(f"QLoRA 어댑터 적용 완료(파인튜닝, F1 0.7020으로 개선 확인): {lora_adapter_path}")

    # Qwen 계열 등에서 답변 도중 중국어로 전환되는 현상 방지 —
    ...
    print(f"LLM 로드 완료 (로컬 GPU, 4bit): {llm_model_name}")
    return llm
