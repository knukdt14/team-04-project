...
OUTPUT_PATH = os.path.join(EVAL_DIR, "llm_comparison.png")
...
def main():
    ...
    labels = [_short_name(m) for m in summary["llm_model"]]

    fig, axes = plt.subplots(1, 3, figsize=(15, 5.5))

    ax = axes[0]
    bars = ax.bar(labels, summary["mean_bertscore_f1"], color="#4C72B0")
    ax.set_title("LLM별 평균 BERTScore F1")
    ...
    ax = axes[2]
    order = summary["llm_model"].tolist()
    data = [df_all.loc[df_all["llm_model"] == m, "bertscore_f1"].values for m in order]
    ax.boxplot(data, tick_labels=[_short_name(m) for m in order])
    ax.set_title("LLM별 질문 단위 F1 분포")
    ax.set_ylabel("BERTScore F1")
    plt.setp(ax.get_xticklabels(), rotation=20, ha="right")

    fig.suptitle("LLM 모델 비교 (자동차관리법 RAG)", fontsize=14)
